...
if( ! class_exists('Farmart_Widget_Layered_Nav') ) {
	/**
	 ...
	 * @extends  WC_Widget
	 */
	class Farmart_Widget_Layered_Nav extends WC_Widget {
		protected $defaults;
		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->widget_cssclass    = 'woocommerce widget_layered_nav woocommerce-widget-layered-nav fm-widget-layered-nav';
			$this->widget_description = esc_html__( 'Display a list of attributes to filter products in your store.', 'farmart-addons' );
			$this->widget_id          = 'farmart_layered_nav';
			$this->widget_name        = esc_html__( 'Farmart - Filter Products by Attribute', 'farmart-addons' );
			parent::__construct();
		}
		...
		public function init_settings() {
			...
			$this->settings = array(
				'title'          => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by', 'farmart-addons' ),
					'label' => esc_html__( 'Title', 'farmart-addons' ),
				),
				'attribute'      => array(
					'type'    => 'select',
					'std'     => '',
					'label'   => esc_html__( 'Attribute', 'farmart-addons' ),
					'options' => $attribute_array,
				),
				'attribute_type' => array(
					'type'    => 'select',
					'std'     => '',
					'label'   => esc_html__( 'Attribute Type', 'farmart-addons' ),
					'options' => array(
						'default' => esc_html__( 'Default', 'farmart-addons' ),
						'list'    => esc_html__( 'List', 'farmart-addons' ),
					),
				),
				'query_type'     => array(
					'type'    => 'select',
					'std'     => 'and',
					'label'   => esc_html__( 'Query type', 'farmart-addons' ),
					'options' => array(
						'and' => esc_html__( 'AND', 'farmart-addons' ),
						'or'  => esc_html__( 'OR', 'farmart-addons' ),
					),
				),
				'search'         => array(
					'type'  => 'checkbox',
					'std'   => '0',
					'label' => esc_html__( 'Search Box', 'farmart-addons' ),
				),
				'height'         => array(
					'type'  => 'text',
					'std'   => '130px',
					'label' => esc_html__( 'Max height.', 'farmart-addons' ),
				),
				'show_dropdown'                     => array(
					'type'  => 'checkbox',
					'std'   => 0,
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $height, $search, $attribute_type ) {
			...
			if ( ! empty( $height ) && $height != '100%' ) {
				$list_class = 'fm-widget-layered-nav-scroll';
			}

			echo '<ul class="woocommerce-widget-layered-nav-list ' . esc_attr( $list_class ) . '" data-height="' . esc_attr( $height ) . '">';

			$term_counts        = farmart_get_filtered_term_product_counts( wp_list_pluck( $terms, 'term_id' ), $taxonomy, $query_type );
			$_chosen_attributes = WC_Query::get_layered_nav_chosen_attributes();
			$found              = false;
