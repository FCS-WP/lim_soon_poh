...
class Walker_Edit extends \Walker_Nav_Menu_Edit {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		$item_icon_text  = get_post_meta( $item->ID, 'tamm_menu_item_icon_text', true );
		$item_icon_color = get_post_meta( $item->ID, 'tamm_menu_item_icon_color', true );
		$item_icon_text_color = get_post_meta( $item->ID, 'tamm_menu_item_icon_text_color', true );
		...
		$mega_fullwidth  = get_post_meta( $item->ID, 'tamm_menu_item_mega_fullwidth', true );
		...
		if ( $settings ) {
			...
			$data->setAttribute( 'data-mega_fullwidth', intval( $mega_fullwidth ) );
			...
			$data->setAttribute( 'data-icon_text', esc_attr( $item_icon_text ) );
			$data->setAttribute( 'data-icon_type', esc_attr( $item_type_icon ) );
			$data->setAttribute( 'data-icon_color', esc_attr( $item_icon_color ) );
			$data->setAttribute( 'data-icon_text_color', esc_attr( $item_icon_text_color ) );
			...
			$item_icon_html = sprintf( '<span class="hidden tamm-data-icons">%s</span>', \Farmart\Icon::sanitize_svg( $item_icon_svg ) );
		...
		if ( $cancel ) {
			...
			$link->nodeValue = esc_html__( 'Settings', 'farmart-addons' );
