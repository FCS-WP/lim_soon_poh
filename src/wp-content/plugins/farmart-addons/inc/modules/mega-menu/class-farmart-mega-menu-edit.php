...
class Edit {
	...
	public function scripts( $hook ) {
		...
		wp_register_style( 'farmart-mega-menu', FARMART_ADDONS_URL . 'inc/modules/mega-menu/assets/mega-menu.css', array(
			'media-views',
			'wp-color-picker',
		), '20160530' );
		wp_enqueue_style( 'farmart-mega-menu' );

		wp_register_script( 'farmart-mega-menu', FARMART_ADDONS_URL . 'inc/modules/mega-menu/assets/mega-menu.js', array(
			'jquery',
			'jquery-ui-resizable',
			'wp-util',
			'backbone',
		...
		wp_enqueue_media();
		wp_enqueue_script( 'farmart-mega-menu' );
	}
            ...
            <div class="tamm-modal media-modal wp-core-ui">
                <button type="button" class="button-link media-modal-close tamm-modal-close">
                    <span class="media-modal-icon"><span
                                class="screen-reader-text"><?php esc_html_e( 'Close', 'farmart-addons' ) ?></span></span>
                </button>
                    ...
                    <div class="tamm-frame-toolbar media-frame-toolbar">
                        <div class="tamm-toolbar media-toolbar">
                            <div class="tamm-toolbar-primary media-toolbar-primary search-form">
                                <button type="button"
                                        class="button tamm-button tamm-button-save media-button button-primary button-large"><?php esc_html_e( 'Save Changes', 'farmart-addons' ) ?></button>
                                <button type="button"
                                        class="button tamm-button tamm-button-cancel media-button button-secondary button-large"><?php esc_html_e( 'Cancel', 'farmart-addons' ) ?></button>
                                <span class="spinner"></span>
                            </div>
                        </div>
                    </div>
	...
	public function templates() {
		...
		foreach ( $templates as $template ) {
			$file = apply_filters( 'tamm_js_template_path', FARMART_ADDONS_DIR . 'inc/modules/mega-menu/tmpl/' . $template . '.php', $template );
			?>
	...
	public function save_menu_item_data() {
		...
		do_action( 'farmart_save_menu_item_data', $data );
