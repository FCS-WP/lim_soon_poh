...
class Settings  {
	...
	const POST_TYPE     = 'farmart_product_tab';
	const OPTION_NAME   = 'farmart_product_tab';
	const TAXONOMY_TAB_TYPE     = 'farmart_product_tab_type';
	...
	public function __construct() {
		...
		add_action( 'farmart_after_product_tab_ordering', array( $this, 'clear_product_tabs_cache' ) );
		...
		add_action( 'wp_ajax_farmart_product_tab_ordering', array( $this, 'product_tab_ordering' ) );
	...
	public function product_tabs_section( $sections ) {
		$sections['farmart_addons_product_tabs'] = esc_html__( 'Product Tabs', 'farmart-addons' );
	...
	public function product_tabs_settings( $settings, $section ) {
		if ( 'farmart_addons_product_tabs' != $section ) {
			return $settings;
		}

		$settings_product_tabs = array(
			array(
				'name' => esc_html__( 'Product Tabs', 'farmart-addons' ),
				'type' => 'title',
				'id'   => self::OPTION_NAME . '_options',
			),
			array(
				'name'    => esc_html__( 'Product Tabs', 'farmart-addons' ),
				'desc'    => esc_html__( 'Enable product tabs manager', 'farmart-addons' ),
				'id'      => self::OPTION_NAME,
				'default' => 'no',
				'type'    => 'checkbox',
				'checkboxgroup' => 'start',
	...
	public function edit_admin_columns( $columns ) {
		$columns = array_merge( $columns, array(
			'product_tab_disable' => esc_html__( 'Disabled', 'farmart-addons' ),
			'product_tab_type' => esc_html__( 'Type', 'farmart-addons' ),
			'product_tab_products' => esc_html__( 'Products', 'farmart-addons' ),
			'product_tab_categories' => esc_html__( 'Categories', 'farmart-addons' ),
		) );
	...
	public function manage_custom_columns( $column, $post_id ) {
		...
		switch ( $column ) {
			case 'product_tab_disable':
				...
				echo $tab_disable == 'yes' ? esc_html__( 'Yes', 'farmart-addons' ) : esc_html__( 'No', 'farmart-addons' );
			...
			case 'product_tab_type':
				...
				if ( ! is_wp_error( $types ) && $types && is_array( $types ) ) {
					...
					switch( $type_name ) {
						case 'global':
							esc_html_e('Global', 'farmart-addons') ;
							break;
						case 'product':
							esc_html_e('Product', 'farmart-addons') ;
							break;
						case 'custom':
							esc_html_e('Custom', 'farmart-addons') ;
							break;
						default:
							esc_html_e('Default', 'farmart-addons') ;
							break;
					}
				} else {
					esc_html_e( 'No Type', 'farmart-addons' );
				}
			...
			case 'product_tab_products':
				...
				if (  $product_ids ) {
					...
					$links[] = esc_html__( 'No Product', 'farmart-addons' );
				}
			...
			case 'product_tab_categories':
				...
				if ( ! is_wp_error( $cats ) && $cats && is_array( $cats ) ) {
					...
					$links[] = esc_html__( 'No Category', 'farmart-addons' );
				}
	...
	public function meta_boxes( $post ) {
		add_meta_box( 'farmart-product-tabs', esc_html__( 'Tabs Settings', 'farmart-addons' ), array( $this, 'tabs_meta_box' ), self::POST_TYPE, 'advanced', 'high' );
	}
	...
	public function tabs_meta_box( $post ) {
		?>
		<div id="farmart-product-tabs-settings" class="farmart-product-tabs-settings">
			<p class="form-field">
				<label><?php esc_html_e('Tab Type', 'farmart-addons'); ?></label>
				<span class="farmart-product-tabs--type">
					<?php
					$terms = get_the_terms( $post->ID, self::TAXONOMY_TAB_TYPE );
					$tab_type = ! is_wp_error( $terms ) && $terms ? $terms[0]->name : 'global';
					?>
					<input type="radio" id="farmart-product-tab-global" <?php echo $tab_type == 'global' ? 'checked' : ''; ?> class="farmart-product-tab--input" checked name="_product_tab_type" value="global">
					<label for="farmart-product-tab-global"><?php esc_html_e('Global', 'farmart-addons'); ?></label>
					<input type="radio" id="farmart-product-tab-product" <?php echo $tab_type == 'product' ? 'checked' : ''; ?> class="farmart-product-tab--input" name="_product_tab_type" value="product">
					<label for="farmart-product-tab-product"><?php esc_html_e('Product', 'farmart-addons'); ?></label>
					<input type="radio" id="farmart-product-tab-custom" <?php echo $tab_type == 'custom' ? 'checked' : ''; ?> class="farmart-product-tab--input" name="_product_tab_type" value="custom">
					<label for="farmart-product-tab-custom"><?php esc_html_e('Custom', 'farmart-addons'); ?></label>
				</span>

			</p>
			<p class="form-field farmart-product-tabs--product">
				<label for="product-tab-id"><?php esc_html_e( 'Products', 'farmart-addons' ); ?></label>
				<select class="wc-product-search" multiple="multiple" style="width: 50%;" id="product-tab-id" name="_product_tab_product_ids[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a product&hellip;', 'farmart-addons' ); ?>" data-action="woocommerce_json_search_products">
					<?php
			...
			<p class="form-field farmart-product-tabs--categories">
				<label for="product_tab_categories"><?php esc_html_e( 'Categories', 'farmart-addons' ); ?></label>
				<select class="wc-category-search" multiple="multiple" style="width: 50%;" id="product_tab_categories" name="_product_tab_cat_slugs[]" data-sortable="true" data-placeholder="<?php esc_attr_e( 'Search for a category&hellip;', 'farmart-addons' ); ?>" data-action="woocommerce_json_search_categories">
					<?php
	...
	public function create_post_type() {
		$posts = array(
			'description' 				=>	esc_html__('Description', 'farmart-addons'),
			'additional_information' 	=>	esc_html__('Additional Information', 'farmart-addons'),
			'reviews' 					=>	esc_html__('Reviews', 'farmart-addons'),
		);
	...
	public static function wc_screen_ids($screen_ids) {
		$screen_ids[] = 'farmart_product_tab';
	...
	public static function edit_custom_box($column_name, $post_type) {
		...
		<fieldset class="inline-edit-col-left">
			<div class="inline-edit-col">
				<label>
					<span class="title"><?php esc_html_e('Disable', 'farmart-addons'); ?></span>
	...
	public static function product_tab_ordering() {
		...
		$menu_orders = wp_list_pluck( $wpdb->get_results( "SELECT ID, menu_order FROM {$wpdb->posts} WHERE post_type = 'farmart_product_tab' ORDER BY menu_order DESC" ), 'menu_order', 'ID' );
		...
		foreach ( $menu_orders as $id => $menu_order ) {
			...
			*/
			do_action( 'farmart_after_single_product_tab_ordering', $id, $index );
		}
		...
		do_action( 'farmart_after_product_tab_ordering', $sorting_id, $menu_orders );
	...
	function product_tab_redirect() {
		global $pagenow;
		if($pagenow == 'post.php' && isset($_GET['post']) && get_post_type( $_GET['post'] ) == 'farmart_product_tab'){
			$terms = get_the_terms( $_GET['post'], self::TAXONOMY_TAB_TYPE );
			$tab_type = ! is_wp_error( $terms ) && $terms ? $terms[0]->name : '';
			if( $tab_type == 'default' ) {
				wp_redirect(admin_url('/edit.php?post_type=farmart_product_tab' ));
				exit;
			}
	...
	public function clear_product_tabs_cache() {
		delete_transient( 'farmart_wc_product_tabs' );
	}
	...
	public function admin_scripts( $hook ) {
		...
		if ( in_array( $hook, array('edit.php', 'post-new.php', 'post.php' ) ) && self::POST_TYPE == $screen->post_type ) {
			wp_enqueue_style( 'farmart-product-tabs', FARMART_ADDONS_URL . 'inc/modules/product-tabs/assets/product-tabs-admin.css' );
			wp_enqueue_script( 'farmart-product-tabs', FARMART_ADDONS_URL . 'inc/modules/product-tabs/assets/product-tabs-admin.js', array( 'jquery', 'jquery-ui-sortable' ),'1.0', true );
